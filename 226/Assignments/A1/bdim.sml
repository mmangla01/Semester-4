...
let exception DivisionByZero
    ...
    val code = Vector.fromList(quadFromString(readFile(filename)))
    (* val dum = print(Int.toString (Vector.length(code))) *)
    ...
    let fun helper(f,g) = let val a = f in  if (g >= Vector.length(code)) then raise NoHaltStatement 
        ...
        fun op3 (i,k) = if (k<0 orelse i<0) then raise NegativeIndexError else if (Array.sub(mem,i) > 1 orelse Array.sub(mem,i)<0) then raise  NonBooleanValue 
